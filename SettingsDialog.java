...
public class SettingsDialog extends JDialog implements ActionListener, ChangeListener {
	...
	//Dialog components
	private JTextField wordlistDirectory, passwordPattern, maxCharLimit, currentCapitializationFrequency, currentSwapPattern;
	...
	private final void setUpComponents() {	
		...
		this.currentCapitializationFrequency = new JTextField(3);
		this.currentCapitializationFrequency.setText(this.capitalizationFrequency + "");
		
		this.currentSwapPattern = new JTextField(3);
		this.currentSwapPattern.setText(this.swapFrequency + "");
		...
		this.manageCapitalization.add(this.currentCapitializationFrequency);
		
		
		// Updating the currentCapitializationFrequency text field by the value of slider as it moves  
		randomCapitalizationFrequency.addChangeListener(new ChangeListener(){ public void stateChanged(ChangeEvent e) { currentCapitializationFrequency.setText(String.valueOf(randomCapitalizationFrequency.getValue())); }  });
		
		// Updating the slider value if the user writes something in the currentCapitializationFrequency
		currentCapitializationFrequency.addKeyListener(new KeyAdapter(){
            
            public void keyReleased(KeyEvent ke) {
                String typed = currentCapitializationFrequency.getText();
                randomCapitalizationFrequency.setValue(0);
                if(!typed.matches("\\d+") || typed.length() > 3) {
                    return;
                }
                int value = Integer.parseInt(typed);
                randomCapitalizationFrequency.setValue(value);
            }
        });
		...
		this.manageCharacterSwapping.add(this.currentSwapPattern);
		
		// Updating the currentSwapPattern text field by the value of slider as it moves
		charSwapFrequency.addChangeListener(new ChangeListener(){ public void stateChanged(ChangeEvent e) { currentSwapPattern.setText(String.valueOf(charSwapFrequency.getValue())); }  });
		
		// Updating the slider value if the user writes something in the currentSwapPattern
		currentSwapPattern.addKeyListener(new KeyAdapter(){
            
            public void keyReleased(KeyEvent ke) {
                String typed = currentSwapPattern.getText();
                charSwapFrequency.setValue(0);
                if(!typed.matches("\\d+") || typed.length() > 3) {
                    return;
                }
                int value = Integer.parseInt(typed);
                charSwapFrequency.setValue(value);
            }
        });
